...
if HAS_MOE_V2_ENHANCED:
    ...
    class MOE_v2_Enhanced_Strategy(BaseStrategy):
        def __init__(self, model_path: str = "models/dynamic_moe_v2_enhanced_final.pkl", **kwargs):
            self.model_path = model_path
            self.model = None
            self.load_model()

        def load_model(self):
            """Load MOE v2 Enhanced model"""
            try:
                import joblib
                if os.path.exists(self.model_path):
                    self.model = joblib.load(self.model_path)
                    logger.info(f"✅ Đã load MOE v2 Enhanced model từ {self.model_path}")
                else:
                    logger.error(f"❌ Không tìm thấy model tại {self.model_path}")
            except Exception as e:
                logger.error(f"❌ Không thể load MOE v2 Enhanced model: {e}")

        def apply_signal_quality_filter(self, df: pd.DataFrame) -> np.ndarray:
            """Áp dụng signal quality filter"""
            try:
                quality_filter = signal_quality_filter(df)
                return quality_filter
            except Exception as e:
                logger.error(f"❌ Không thể áp dụng signal quality filter: {e}")
                return np.ones(len(df), dtype=bool)

        def predict(self, df: pd.DataFrame) -> np.ndarray:
            """Dự đoán tín hiệu từ dữ liệu"""
            try:
                ...
                if not all(feature in df.columns for feature in feature_names):
                    logger.error("❌ Thiếu features cần thiết cho model")
                    return np.zeros(len(df))
                ...
                # Áp dụng signal quality filter
                quality_filter = self.apply_signal_quality_filter(df)
                predictions = predictions * quality_filter.astype(int)
...
def load_config_from_env() -> BotConfig:
    ...
    if strategy_name == "moe_v2_enhanced":
        moe_defaults = {
            ...
            "proba_threshold": _float_env("MOE_PROBA_THRESHOLD", 0.35),
